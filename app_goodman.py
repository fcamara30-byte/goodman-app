...
def FS_material(mat,f):
    ...
    elif mat=="HS propietario": return f*0.83
...
with r:
    ...
    ax.set_xlabel("Smin (ksi)"); ax.set_ylabel("Smax (ksi)")
    ax.grid()
    plt.tight_layout()
    st.pyplot(fig)
    ...
    # LOGICA
    if f_base==1:
        orden=["HS97","HA96","DSK75","CS propietario","D New","DA78"]
        df["orden"]=df["Material"].apply(lambda x: orden.index(x))
        df=df.sort_values("orden")
    else:
        df=df.sort_values("Margen",ascending=False)
    ...
    st.dataframe(df.drop(columns=["orden"],errors="ignore").style.format({
        "FS":"{:.3f}",
        "Sadm":"{:.1f}",
        "Margen":"{:.1f}",
        "%Goodman":"{:.1f}"
