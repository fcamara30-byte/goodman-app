...
if len(df) > 1:
    ...
    for i in range(1, len(df)):
        ...
        dinc = df["inc"][i] - df["inc"][i-1]
        dls.append(abs(dinc) * 100 / max(dmd,1))
    ...
    colores = ["green" if d<=2 else "yellow" if d<=4 else "red" for d in df["DLS"]]
    ...
    for k in range(120):
        ...
        impacto = (np.cos(theta) > 0.98) & crit_mask
